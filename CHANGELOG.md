...
## 1.6.3
- Fixed: 修复预设条目在部分酒馆美化主题下开关被合成层遮挡或失效的问题，修复开关区域误触发分组的问题。
- Added: 预设条目“更多操作”菜单新增“删除条目”功能，可一键删除提示词条目。
- Added: 转移工具折叠栏新增“条目更多操作”功能开关，可按需开启或隐藏该按钮。
- Changed: 重构预设条目分组头与正则脚本分组头样式为通透简约的磨砂玻璃风格，适当增加头部高度。
- Changed: 预设条目更多操作菜单与分组更多菜单重构为磨砂玻璃质感。
- Changed: 优化转移工具折叠栏视觉，去除纯色底色，标签栏改用简约线条风格。

## 1.6.2
- Fixed: 修复预设出现无法解析的旧分组时导致条目无法加入已有分组的问题。
- Fixed: 修复预设转移界面分组选中问题，优化体验。

## 1.6.1
- Fixed: 重写“快照管理 -> 编辑快照”弹窗，改为复用通用 modal 布局，修复桌面端与手机端显示不稳定问题。
- Changed: 清理快照编辑弹窗遗留 CSS，只保留快照列表与条目样式。
- Changed: 更多操作按钮与分组更多按钮的尺寸逻辑继续对齐酒馆原生 edit/remove，菜单字号跟随酒馆主题字号。

## 1.6.0
- Added: 新增预设条目美化正则功能，自动为条目制作前端美化。
- Added: 新增原生面板条目复制按钮。
- Added: 快照管理面板新增快照数据导出/导入以及手动保存快照功能，并优化相关交互可手动编辑快照。
- Added: 正则分组功能扩展到预设正则和局部正则。
- Changed: 预设条目分组在复制、移动、编辑等操作适配原有分组。
- Changed: 优化预设转移ui界面的操作体验。
- Changed: 优化快照存储与缝合同步逻辑，改进状态面板中未分组条目的展示方式。
- Removed: 移除预设比较中的更新日志生成功能及相关界面入口。

## 1.5.6
- Added: 新增"收藏条目"功能，支持跨预设收藏常用条目，并通过收藏面板快速转移到任意预设。
- Added: 新增 Changelog 自动生成工具，可对比预设自动生成版本更新日志草稿。
